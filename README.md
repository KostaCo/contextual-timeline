...
# Contextual Timeline – Add Your Own Data
...
1. In `data.js`, add a new group to the `timelineGroups` array:

   ```js
   { id: "movies", content: "🎬 Movies" }
   ```

2. Add your items with that group name:

   ```js
   { id: 4001, group: "movies", content: "My Movie", start: "1999-01-01", className: "movie" }
   ```

3. In `style.css`, add styles for your new group (copy the `.book`, `.history`, or `.person` blocks and rename).

4. In `script.js`, add your group to the `groupsData` object inside `updateTimelineItems()` so it gets clustered:
